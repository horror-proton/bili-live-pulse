// modified from https://socialsisteryi.github.io/bilibili-API-collect/docs/misc/sign/wbi.html
...
fn _encode_wbi(
    ...
    let query = params
        ...
        .join("&");
    // 计算签名
    let web_sign = hex::encode(
        hash(
            MessageDigest::md5(),
            (query.clone() + &mixin_key).as_bytes(),
        )
        .unwrap(),
    )
    .replace(" ", "")
